 ...
 */
    ...
    // let mode = prompt('请输入相应数字以选择游戏模式：\n【1】合成豆苗苗普通模式（不输入或其他输入均默认选择此模式）\n【2】合成豆苗苗无敌模式\n【3】合成苗豆豆普通模式\n', '1').trim();
    // reverseLevelUp = false;
    // switch(mode)
    // {
    //     default:
    //     case '1':
    //         document.getElementsByTagName("title")[0].innerText = '合成豆苗苗普通模式';
    //         break;
    //     case '2':
    //         wuDi = false;  
    //         document.getElementsByTagName("title")[0].innerText = '合成豆苗苗无敌模式';
    //         break; 
    //     case '3':
    //         firstFruit = 10;  
    //         reverseLevelUp = true;
    //         document.getElementsByTagName("title")[0].innerText = '合成苗豆豆普通模式';
    //         break;
    // }
    // 指定生成的水果：默认值: 不开启反转 0-5 开启反转 6-11，修改对应数字即可控制随机生成的水果范围
    minRandomFruitNum = reverseLevelUp ? 5 : 0; // 生成随机水果最小值（0-10）0 为葡萄，9 为半个西瓜
    maxRandomFruitNum = reverseLevelUp ? 9 : 5; // 生成随机水果最大值（1-11）0 为葡萄，9 为半个西瓜
    setFruits = {
        // 指定前几次生成的水果，可填入任意数量的数字，0 为葡萄，9 为半个西瓜
        startFruits: reverseLevelUp ? [10, 9, 9, 9, 8, 7] : [0, 0, 1, 2, 2, 3],
        randomFunction: () => {
